...
{
	public class ConvertToProfileStats
	{
		public ProfileStats Convert(string content)
		{
			ProfileStats ps = Initialize();
			try
			{
				ps = JsonConvert.DeserializeObject<ProfileStats>(content);
			}
			catch (Exception ex)
			{
		...
		private ProfileStats Initialize()
		{
