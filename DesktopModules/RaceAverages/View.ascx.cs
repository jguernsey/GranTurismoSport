...
namespace Jjg.GtsStats.RaceAverages
{
	public partial class View : RaceAveragesModuleBase, IActionable
	{
		private void LoadData()
		{
			...
			{
				GetRaceAverages gra = new GetRaceAverages();
				DataTable dt = gra.Get(userName);
				if (dt.Rows.Count > 0)
				{
					dgvTrackAverages.DataSource = dt;
					dgvTrackAverages.DataBind();
				}
			}
		}
		protected void OnDataBound(object sender, EventArgs e)
		{
			for (int i = 0; i < dgvTrackAverages.Rows.Count; i++)
			{
				GridViewRow row = dgvTrackAverages.Rows[i];
